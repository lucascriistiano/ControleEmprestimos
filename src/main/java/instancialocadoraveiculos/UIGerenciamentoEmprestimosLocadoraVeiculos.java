...
public class UIGerenciamentoEmprestimosLocadoraVeiculos implements UIGerenciamentoEmprestimos {
	...
	private GerenciadorEmprestimos gerenciadorEmprestimos = new GerenciadorEmprestimos(new RegraLocadoraVeiculos(),
	...
	public void realizarEmprestimo() {
		...
		try {
			...
			ComprovanteEmprestimo comprovanteEmprestimo = gerenciadorEmprestimos.realizarEmprestimo(usuario, cliente,
					recursos, dataDevolucao);
			...
			System.out.println(
					"Ocorreu um erro ao realizar o emprestimo. Verifique se dados foram inseridos corretamente. Erro: "
							+ e.getMessage());
		}
	}
	...
	public void realizarDevolucao() {
		...
		try {
			...
			for (Recurso recurso : emprestimo.getRecursos()) {
				...
				do {
					...
					if (quilometragemAtual < quilometragemInicial)
						System.out.print(
								"A quilometragem atual do veiculo nao pode ser menor do que no momento do emprestimo.");
			...
			System.out.println(
					"Ocorreu um erro ao realizar a devolucao. Verifique se dados foram inseridos corretamente.");
		}
	}
			...
			if (emprestimos.size() > 0) {
				for (Emprestimo emprestimo : emprestimos) {
					...
					System.out.println(" - Data devolucao: "
							+ new SimpleDateFormat("dd/MM/yyyy").format(emprestimo.getDataDevolucao()));
	...
	public void verificarPrazos() {
		try {
			...
			if (this.gerenciadorEmprestimos.verificarPrazos()) { // Existiram
																	// emprestimos
																	// notificados
